...
public class TestRBM {
	...
	private final float[][] reconstrX = new float[testN][nVisible];
	...
	public void doTest() {
		...
		// construct RBM
		RBM nn = new RBM(nVisible, nHidden, null, null, null, rand);

		// train with contrastive divergence
		for (int epoch = 0; epoch < epochs; epoch++) {
			for (int batch = 0; batch < miniBatchN; batch++) {
				nn.contrastiveDivergence(trainMiniBatch[batch], miniBatchSize, learningRate, 1);
			}
			learningRate *= 0.995;
		}
		...
		for (int i = 0; i < testN; i++) {
			reconstrX[i] = nn.reconstruct(testX[i]);
		}
